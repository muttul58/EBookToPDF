...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QLabel, 
                             ...
                             QSizePolicy, QProgressBar, QCheckBox, QButtonGroup, QRadioButton, 
                             QSpacerItem)  
from PyQt5.QtGui import QPixmap, QPainter, QPen, QColor, QFont, QIcon
from PyQt5.QtCore import Qt, QRect, QPoint, QSize, QUrl, QThread, pyqtSignal
...
def show_message(self, title, message):
    # 메인 윈도우를 부모로 지정하여 메시지 박스 표시
    QMessageBox.information(self, title, message)

# 매크로 실행 중지 관련 
class MacroThread(QThread):
    message_signal = pyqtSignal(str, str)  # 메시지를 전달하기 위한 시그널
    progress_signal = pyqtSignal(int)  # 진행상황을 전달하기 위한 시그널
    progress_label_signal = pyqtSignal(str)  # 라벨 업데이트를 위한 시그널 추가
    
    def __init__(self, parent=None, repeat_count=1, wait_time=0.5, click_position=(0,0), 
                 selected_window=None, repeat_spinbox=None, time_slider=None):
        ...
        self.repeat_spinbox = repeat_spinbox
        self.time_slider = time_slider
        self.macro_running = True
        self.macro_progress_bar = None
        self.macro_progress_label = None
    
    def run(self):
        if not self.selected_window:
            self.message_signal.emit("경고", "캡처할 창을 선택해주세요.")
            return
        ...
        try:
            ...
            for i in range(self.repeat_count):
                ...
                if self.macro_progress_bar:
                    self.progress_signal.emit(i + 1)
                if self.macro_progress_label:
                    self.macro_progress_label.setText(f"{i+1}/{self.repeat_count}")
                # 프로그레스 바와 라벨 업데이트
                self.progress_signal.emit(i + 1)
                self.progress_label_signal.emit(f"{i+1}/{self.repeat_count}")

            if self.macro_running:
                self.message_signal.emit("매크로 완료", "매크로 실행이 완료되었습니다.")
            
        except Exception as e:
            error_msg = f"매크로 실행 중 오류가 발생했습니다: {str(e)}\n\n"
            error_msg += traceback.format_exc()
            self.message_signal.emit("오류", error_msg)
...
class IntegratedEBookApp(QMainWindow):
    def __init__(self):
        ...
        self.macro_thread = None  # 매크로 스레드 인스턴스 초기화
        self.setWindowTitle("eBook 캡처 및 PDF 생성기 by muttul Ver37(2025.0104.1714)")  # 윈도우 제목을 설정합니다.
        ...
        self.help_tab = QWidget()  # 사용 설명 탭 위젯을 생성합니다.
        self.help_layout = QVBoxLayout(self.help_tab)  # 사용 설명 탭에 수직 박스 레이아웃을 설정합니다.
        self.tab_widget.addTab(self.help_tab, "사용 방법")  # 사용 설명 탭을 탭 위젯에 추가합니다.

        # 각 탭의 UI를 설정합니다.
        self.setup_capture_tab()  # 캡처 탭 UI를 설정하는 메서드를 호출합니다.
        self.setup_pdf_tab()  # PDF 탭 UI를 설정하는 메서드를 호출합니다.
        self.setup_help_tab()  # 사용 설명 탭 UI를 설정하는 메서드를 호출합니다.
    ...
    # 사용 방법 탭 UI를 설정하는 메서드입니다.
    def setup_help_tab(self):
        self.help_layout = QVBoxLayout()  # 사용 방법 탭의 레이아웃을 수직 박스 레이아웃으로 설정합니다.
        #self.help_layout.setSpacing(20)  # 레이아웃 내 위젯 간 간격을 20픽셀로 설정합니다.

        # "사용 설명 동영상 보기" 버튼 추가
        self.video_button = QPushButton('사용 설명 동영상 보기', self)
        self.video_button.clicked.connect(self.open_video)  # 버튼 클릭 시 open_video 메서드를 호출
        ...
        self.video_button.setStyleSheet("""
            QPushButton {
                ...
                border-radius: 4px;         /* # 버튼 모서리를 4픽셀 둥글게 만듭니다. */
                min-width: 350px;           /* 버튼의 최소 너비를 150픽셀로 설정합니다. */
                min-height: 50px;           /* 버튼의 최소 높이를 50픽셀로 설정합니다. */
                max-width: 500px;           /* 버튼의 최대 너비를 300픽셀로 설정합니다. */
                max-height: 50px;           /* 버튼의 최대 높이를 50픽셀로 설정합니다. */
                align-items: center;        /* 버튼을 중앙에 정렬합니다. */
                font-size: 20px;            /* 버튼 텍스트 크기를 20픽셀로 설정합니다. */
            }
            QPushButton:hover {
        ...
        # 버튼을 레이아웃에 추가 및 중앙 정렬 설정
        self.help_layout.addStretch(1)  # 남은 공간을 채웁니다.
        self.help_layout.addWidget(self.video_button, 0, Qt.AlignCenter)  # 0은 스페이싱, Qt.AlignCenter로 중앙 정렬
        self.help_layout.addStretch(1)  # 남은 공간을 채웁니다.

        # 사용 설명 탭에 레이아웃을 설정합니다.
        help_widget = QWidget()  # 새로운 위젯을 생성합니다.
        help_widget.setLayout(self.help_layout)  # 위젯에 사용 설명 탭 레이아웃을 설정합니다.
        self.help_tab.setLayout(QVBoxLayout())  # 사용 설명 탭에 수직 박스 레이아웃을 설정합니다.
        self.help_tab.layout().addWidget(help_widget)  # 사용 설명 탭 레이아웃에 위젯을 추가합니다.

        # 사용 설명 탭에 레이아웃을 설정합니다.
        if self.help_tab.layout() is None:
            self.help_tab.setLayout(QVBoxLayout())
        self.help_tab.layout().addWidget(help_widget)

    # 사용 설명 동영상 보기 버튼을 클릭했을 때 호출되는 메서드입니다.
    def open_video(self):
        # 링크 열기
        url = "https://youtu.be/srk8iUTtb-4"
        webbrowser.open(url)  # 웹 브라우저를 사용하여 URL을 엽니다.
    ...
    def setup_capture_tab(self):
        ...
        # 기존 레이아웃 제거
        if self.capture_tab.layout():  # 레이아웃이 없는 경우에만 새로 설정
            QWidget().setLayout(self.capture_tab.layout())

        # 새 레이아웃 설정
        self.capture_layout = QVBoxLayout()
        self.capture_layout.setSpacing(20)
        ...
        # 매크로 중지 버튼을 생성합니다.
        self.stop_button = QPushButton("매크로 중지", self)  # "매크로 중지" 버튼을 생성합니다.
        self.stop_button.clicked.connect(self.stop_macro)  # 버튼 클릭 시 stop_macro 메서드를 호출합니다.
        self.stop_button.setStyleSheet("""
            QPushButton {
                background-color: #ff0000;  /* # 버튼 배경색을 빨간색으로 설정합니다. */
                color: white;               /* # 버튼 텍스트 색상을 흰색으로 설정합니다. */
                padding: 10px;              /* # 버튼 내부에 10픽셀의 패딩을 추가합니다. */
                border-radius: 4px;         /* # 버튼 모서리를 4픽셀 둥글게 만듭니다. */
            }
            QPushButton:hover {
                background-color: #8B0000;  /* # 마우스를 올렸을 때 버튼 배경색을 약간 어둡게 변경합니다. */
            }
        """)  # 버튼의 스타일을 설정합니다.
        self.capture_layout.addWidget(self.stop_button)  # 매크로 중지 버튼을 캡처 탭 레이아웃에 추가합니다.
    ...
    # start_macro 메서드에서 MacroThread 인스턴스를 생성할 때 인자 전달
    def start_macro(self):
        if self.macro_thread and self.macro_thread.isRunning():
            QMessageBox.warning(self, "경고", "이미 실행 중인 매크로가 있습니다.")  # 매크로 실행 중 알림
            return

        # 매크로 스레드 인스턴스 생성 시 값 전달
        ...
        # 프로그레스 바 초기화
        self.macro_progress_bar.setRange(0, self.repeat_spinbox.value())
        self.macro_progress_bar.setValue(0)
        self.macro_progress_bar.setVisible(True)
        self.macro_progress_label.setVisible(True)
        self.macro_progress_label.setText(f"0/{self.repeat_spinbox.value()}")


        # 시그널 연결
        self.macro_thread.message_signal.connect(lambda title, msg: QMessageBox.information(self, title, msg))
        self.macro_thread.progress_signal.connect(lambda value: self.macro_progress_bar.setValue(value))
        self.macro_thread.progress_label_signal.connect(self.macro_progress_label.setText)
    ...
    def on_macro_finished(self):
        self.macro_progress_bar.setVisible(False)  # 매크로 종료 후 프로그레스 바 숨김
        self.macro_progress_label.setVisible(False)  # 레이블 숨김
        QMessageBox.information(self, "완료", "매크로 실행이 완료되었습니다.")

    def stop_macro(self):
        if self.macro_thread and self.macro_thread.isRunning():
            self.macro_thread.stop()  # 매크로 스레드 중지 요청
            self.macro_thread.wait()  # 스레드가 안전하게 종료될 때까지 대기
            QMessageBox.information(self, "매크로 중지", "매크로가 중지되었습니다.")  # 중지 메시지 표시
        else:
            QMessageBox.warning(self, "경고", "현재 실행 중인 매크로가 없습니다.")  # 실행 중인 매크로 없을 때 알림
    ...
    def on_macro_finished(self):
        QMessageBox.information(self, "완료", "매크로 실행이 완료되었습니다.")  # 매크로 실행 완료 알림
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                for i, img_file in enumerate(images):
                    try:
                        ...
                        # 저장할 디렉토리 생성
                        save_dir = os.path.join(self.cropper_folder, "cropped_images")
                        os.makedirs(save_dir, exist_ok=True)
                        
                        # 크롭된 이미지 저장
                        cropped_img_path = os.path.join(save_dir, f"cropped_image_{i+1}.png")
                        ...
                        self.progress_label.setText(f"처리 중: {i + 1}/{total_steps}\n{img_file}")
    ...
    def on_tab_changed(self, index):
        if index == 1 and not hasattr(self, 'is_pdf_tab_initalized'):
            self.initialize_pdf_tab()
            self.if_pdf_tab_initialized = True
